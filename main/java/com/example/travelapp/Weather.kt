...
import com.example.travelapp.cityFinder
...
import com.example.travelapp.weatherData
...
class Weather : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mCityFinder = findViewById<View>(R.id.cityFinder) as RelativeLayout?
        NameofCity = findViewById(R.id.cityName)
        mCityFinder?.setOnClickListener(View.OnClickListener {
            val intent = Intent(this@Weather, cityFinder::class.java)
            startActivity(intent)
        })
        val backbutton3 = findViewById<ImageView>(R.id.backButton3)
        backbutton3.setOnClickListener {
            val backi = Intent(this, MainActivity::class.java)
                    startActivity(backi)
                }
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == REQUEST_CODE) {
            if (grantResults.size > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                Toast.makeText(this@Weather, "Locationget Succesffully", Toast.LENGTH_SHORT).show()
                weatherForCurrentLocation
            } else {
